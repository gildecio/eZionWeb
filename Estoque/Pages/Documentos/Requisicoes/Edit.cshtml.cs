...
public class EditModel : PageModel
{
    ...
    {
        var item = _docs.GetRequisicoes().FirstOrDefault(d => d.Id == id);
    ...
    public IActionResult OnPost()
    {
        ...
        try
        {
            var doc = new RequisicaoEstoque { Id = Id, ProdutoId = ProdutoId, LocalOrigemId = LocalOrigemId, UnidadeId = UnidadeId, Quantidade = Quantidade, Observacao = Observacao };
            _docs.UpdateRequisicao(doc);
            return RedirectToPage("Index");
        }
        catch (Exception ex)
        {
